# Sherlocks Very Easy

Desafíos de forensics para principiantes. Introducción al análisis de evidencia y lectura de logs.

## Vista General de Dificultad

- **Tiempo:** 15 - 30 minutos
- **Habilidades:** Lectura básica de logs, análisis de archivos, grep/herramientas simples
- **Enfoque:** Comprensión de conceptos forenses, correlación básica de evidencia

---

## 📈 Progreso

🟦🟦⬜⬜⬜ | **5/20 completados (25%)**

---

## 📋 Desafíos

| # | Desafío | Categoría | Estado |
|---|-----------|----------|--------|
| 01 | [Brutus](01_Brutus/readme.md) | Forensics / Log Analysis | ✅ 8/8 |
| 02 | [DreamJob-2](02_DreamJob/readme.md) | Malware Analysis / Operation North Star | ✅ 13/13 |
| 03 | [RomCom](03_RomCom/readme.md) | DFIR / WinRAR Vulnerability | ✅ 10/10 |
| 04 | [PhishNet](04_PhishNet/readme.md) | Email Security & Phishing Analysis | ✅ 11/11 |
| 05 | [SalineBreeze-1](05_SalineBreeze/readme.md) | Threat Intelligence / MITRE ATT&CK | ✅ 20/20 |

---

## Consejos

- Lee las preguntas cuidadosamente
- Busca logs sistemáticamente
- Usa grep para búsquedas rápidas
- Documenta cada paso

---

## 🔗 Volver a Sherlocks

[← Sherlocks](../readme.md) | [← HackTheBox](../../../readme.md)

---

**Última Actualización:** 29 de diciembre de 2025
